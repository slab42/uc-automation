...
def read_clusters_csv(clusters_file):
    """Read cluster definitions from CSV file (cluster_name, server, version)."""
    clusters = []
    try:
...
def get_object_for_single_operation(basepath, service):
    """
    ...
    if service.upper() == 'CUBE':
        ...
        if not server:
            ...
            return None
        return {'name': name, 'server': server, 'version': version}


def get_objects_for_multi_operation(basepath, service):
    """
