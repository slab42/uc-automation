...
The routePartition and newRoutePartition columns are optional. If either
column is missing from the CSV header, the corresponding value below is
...
routePartition = 'Enterprise_PT'
newRoutePartition = 'Migrated_to_WbxCalling'
...
def use_csv(reverse=False):
    ...
    with open(input_file, 'r', encoding='utf8') as my_file:
        ...
        has_new_route_partition_col = 'newRoutePartition' in (csv_file.fieldnames or [])
        for row in csv_file:
            pattern = row['pattern']
            route_partition_name = row['routePartition'] if has_route_partition_col else routePartition
            new_route_partition_name = row['newRoutePartition'] if has_new_route_partition_col else newRoutePartition
            if reverse:
                route_partition_name, new_route_partition_name = new_route_partition_name, route_partition_name
            move_dn_partition(pattern, route_partition_name, new_route_partition_name)
