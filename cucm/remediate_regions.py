...
Lists regions and their dependencies, then optionally deletes regions with no dependencies
Excludes system regions like Default
...
SYSTEM_REGIONS = {'Default'}
MAX_WORKERS = 10


def fetch_region_dependencies(axl, region_name):
    """Fetch dependencies for a single region (used in parallel execution)"""
    deps_result = axl.find_region_dependencies(region_name)
    return {
...
def display_regions(axl, logger, script_dir, debug_mode=False):
    """List all regions and their dependencies"""
    search_term = input("Enter region name to search (or press Enter to list all): ").strip()
    no_dependency_regions = []

    logger.info('Fetching regions...')
    regions_result = axl.list_Regions()

    if not regions_result.get('success'):
        logger.error('Failed to fetch regions: %s', regions_result.get('error'))
        print(f"Error: {regions_result.get('error')}")
        return []

    regions = regions_result.get('response')
    ...
    # Filter out system regions and apply search in one pass
    filtered_regions = []
    for region in regions:
        region_name = region.get('name', 'Unknown')
        if region_name not in SYSTEM_REGIONS:
            if not search_term or search_term.lower() in region_name.lower():
                filtered_regions.append(region)

    if not filtered_regions:
        if search_term:
            logger.info('No regions found matching "%s"', search_term)
            print(f"No regions found matching '{search_term}'")
        else:
            logger.info('No custom regions found (only system regions)')
            print("No custom regions found (system regions excluded)")
        return []

    regions = filtered_regions
    if search_term:
        logger.info('Found %d regions matching "%s"', len(regions), search_term)
    else:
        logger.info('Found %d custom regions', len(regions))

    print(f"\n{'='*80}")
    print(f"Regions ({len(regions)} regions)")
    print(f"{'='*80}\n")

    logger.info('Fetching dependencies for %d regions in parallel', len(regions))

    # Fetch all dependencies in parallel
    region_deps = {}
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {
            executor.submit(fetch_region_dependencies, axl, region.get('name', 'Unknown')): region
            for region in regions
        }
        ...
        for future in as_completed(futures):
            try:
                ...
                region_deps[result['region_name']] = result
            except Exception as e:
                region = futures[future]
                region_name = region.get('name', 'Unknown')
                logger.error('Failed to fetch dependencies for %s: %s', region_name, str(e))
    ...
    for region in regions:
        ...
        if region_name not in region_deps:
            logger.warning('No dependency result for region: %s', region_name)
            print(f"Region: {region_name}")
            print(f"  Error: Failed to fetch dependencies")
            print()
            continue

        result = region_deps[region_name]
        deps_result = result['deps_result']
        dependencies = result['dependencies']

        if debug_mode:
            logger.info('Region: %s - Dependencies result: %s', region_name, deps_result)
        ...
        if dependencies:
            ...
            no_dependency_regions.append({
                'name': region_name
            })
        print()

    return no_dependency_regions


def run_operation_on_cluster(script_dir, cluster_data, cluster_credentials, logger, debug_mode=False):
    """Run the region listing on a single cluster"""
    ...
    try:
        ...
        no_dep_regions = display_regions(axl, logger, script_dir, debug_mode=debug_mode)

        logger.info('Completed cluster: %s', cluster_name)
        print(f"✓ Completed {cluster_name} ({server})")
        return (True, no_dep_regions, cluster_name, axl)
    except Exception as e:
...
def run_on_all_clusters(script_dir, clusters_data, logger, debug_mode=False):
    ...
    for cluster in clusters_data:
        ...
        success, no_dep_regions, cluster_name, axl = result
        if success:
            successful += 1
            for region in no_dep_regions:
                region['cluster'] = cluster_name
            all_no_dep_regions.extend(no_dep_regions)
    ...
    return all_no_dep_regions, cluster_axl_map


def delete_regions(regions, cluster_axl_map, logger):
    """Delete regions organized by cluster"""
    if not regions:
        print("No regions to delete")
        return 0, 0

    print(f"\n{'='*80}")
    print(f"Regions to Delete ({len(regions)} regions)")
    print(f"{'='*80}\n")

    clusters_set = set()
    for region in regions:
        print(f"  - {region.get('name')} (Cluster: {region.get('cluster')})")
        clusters_set.add(region.get('cluster'))
    ...
    clusters_data = {}
    for region in regions:
        cluster_name = region.get('cluster')
        if cluster_name not in clusters_data:
            clusters_data[cluster_name] = []
        clusters_data[cluster_name].append(region)

    successful_deletes = 0
    failed_deletes = 0

    # Delete regions by cluster
    print("\nDeleting regions...\n")
    for cluster_name, cluster_regions in clusters_data.items():
        if cluster_name not in cluster_axl_map:
            logger.warning('No AXL client for cluster %s', cluster_name)
            failed_deletes += len(cluster_regions)
            continue
        ...
        for region in cluster_regions:
            ...
            try:
                ...
                if result.get('success'):
                    ...
                    logger.info('Deleted region: %s', region_name)
                    successful_deletes += 1
                else:
                    error_msg = result.get('error', 'Unknown error')
                    print(f"  ✗ Failed: {region_name} - {error_msg}")
                    logger.error('Failed to delete region %s: %s', region_name, error_msg)
                    failed_deletes += 1
            except Exception as e:
                print(f"  ✗ Exception: {region_name} - {str(e)}")
                logger.error('Exception deleting region %s: %s', region_name, str(e))
                failed_deletes += 1
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Remediate Regions - List and Delete Without Dependencies')
    ...
    log_file = f"_logs/{timestamp}-remediate-regions.log"
    logger = setup_logger(log_file, debug=args.debug)
    logger.info("Remediate Regions - Started")
    ...
    if clusters_data:
        ...
        if use_multiple:
            all_no_dep_regions, cluster_axl_map = run_on_all_clusters(script_dir, clusters_data, logger, debug_mode=args.verbose)
        else:
            ...
            no_dep_regions = display_regions(axl, logger, script_dir, debug_mode=args.verbose)
            for region in no_dep_regions:
                region['cluster'] = cluster['name']
            all_no_dep_regions = no_dep_regions
    ...
    # Offer to delete regions with no dependencies
    if all_no_dep_regions:
        successful, failed = delete_regions(all_no_dep_regions, cluster_axl_map, logger)
        ...
        print("\nNo regions without dependencies to delete")

    logger.info("Remediate Regions - Completed")
