...
def remove_enterpriseAltNum_line(axl, logger, pattern, route_partition_name):
...
def run_single_enterpriseAltNum_line(axl, logger):
    """
    Update single DN to remove Alternate Number
    """
    pattern = input('Pattern: ')
    route_partition_name = input('Route Partition Name: ')
    remove_enterpriseAltNum_line(axl, logger, pattern, route_partition_name)


def run_csv_file(axl, logger, csv_file_path):
    """
    Bulk Remove Enterprise Alternate Numbers from DNs in CSV
    """
...
def run_operation_on_cluster(basepath, cluster_data, operation_params, cluster_credentials, logger):
    """Run enterprise alternate number removal on a single cluster."""
    try:
...
def run_on_all_clusters(basepath, clusters_data, operation_params, logger):
    """Run enterprise alternate number removal on all clusters sequentially."""
...
if __name__ == '__main__':
    ...
    log_file = f"../_logs/{timestamp}-remove-dn-enterprisealternatenumber.log"
