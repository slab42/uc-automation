...
def uncallForwardAll(locationID,ownerType):
    global invalidOwner
    ### Person:
    if ownerType == 'PEOPLE':
    ...
    if invalidOwner == 'Yes':
        ...
        if ownerType == 'VIRTUAL_LINE':
            payload = json.dumps({
                "callForwarding": { "always": { "enabled": False, "destination": '' }},
                "businessContinuity": { "enabled": False, "destination": "" }
                })
        # elif ownerType == 'HUNT_GROUP' or ownerType == 'CALL_QUEUE' or ownerType == 'AUTO_ATTENDANT':
        #     payload = json.dumps({ "callForwarding": { "always": { "enabled": True, "destination": f"{forwardNumber}" }, "operatingModes": { "modes": [] } }, })
        else:
            payload = json.dumps({ "callForwarding": { "always": { "enabled": False, "destination": "" }}})
...
with open(input_file, 'r', encoding='utf-8-sig') as my_file:
    ...
    for index, row in enumerate(csv_file, start=2):
        ...
        if phoneNumber:
            ...
            unforwardThisNumber = uncallForwardAll(locationID,ownerType)

            if unforwardThisNumber.status_code != 204:
                print(f'ERROR: Row {index}, Response Code: {unforwardThisNumber.status_code}\n {unforwardThisNumber.text}')
            else:
                print(f'INFO: Phone number {phoneNumber} successfully removed call forward all.')
