...
huntGroupListUrl = 'https://webexapis.com/v1/telephony/config/huntGroups'
...
def virtualLineLookup(dn):
  response = requests.request("GET", virtualLineLookupUrl + dn, headers=defaultHeaders)
  if response.status_code == 200:
...
# Get List of Hunt Groups and check to make sure it is not 0.
response = requests.request("GET", huntGroupListUrl, headers=defaultHeaders)
if response.status_code != 200 or len(response.json()['huntGroups']) == 0:
    print(response.status_code)
    print(response.text)
    quit()

# Break Json into readable list for menu
for groups in response.json()['huntGroups']:
    huntGroupList.append(groups)

# Create a menu of Hunt Groups to Choose From
print()
print('Choose a Hunt Group')
print('------------------')
for index, item in enumerate(huntGroupList, start=1):
...
for groups in huntGroupList:
    if groups['name'] == groupSelected:
       print('INFO: ' + groups['name'] + ' selected in location ' + groups['locationName'])
       huntGroupId = groups['id']
       locationId = groups['locationId']
       break

# Get a list of IDs for Hunt Group
for entry in emailsAndDns:
   if entry.isnumeric():
...
huntGroupUpdateUrl = f'https://webexapis.com/v1/telephony/config/locations/{locationId}/huntGroups/{huntGroupId}'
response = requests.request("PUT", huntGroupUpdateUrl, headers=headers, data=payload)
